...
def add_mesh(polygons, mesh):
    ...
    for i in range(len(f)):
        if len(f[i]) > 0:
            if f[i][0] == 'v':
                coords = f[i].split(" ")[1:]
                vertice_list.append([float(coord) for coord in coords])
            if f[i][0] == 'f':
                face = f[i].split(" ")[1:]
                face = [i.split('/')[0] for i in face]
                face_list.append([int(i) - 1 for i in face])
    for face in face_list:
        if len(face) == 3 or len(face) == 4:
            p0 = vertice_list[face[0]]
            p1 = vertice_list[face[1]]
            p2 = vertice_list[face[2]]
            add_polygon(polygons, p0[0], p0[1], p0[2], p1[0], p1[1], p1[2], p2[0], p2[1], p2[2])
            if len(face) == 4:
                p3 = vertice_list[face[3]]
                add_polygon(polygons, p0[0], p0[1], p0[2], p2[0], p2[1], p2[2], p3[0], p3[1], p3[2])
    return mesh
